...
logger = logging.getLogger("rpicam_auto_v3")
...
# 새로 보완: 얼굴/각도/거리 안정화 프레임
STABLE_FACE_HOLD_FRAMES = 6
# 새로 보완: 열린 눈 기준 EAR 샘플 수집
...
def draw_face_guide(cam2_bgr, detection):
    ...
    cv2.putText(out, detection["guide_text"], (20, h - 110), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    cv2.putText(
        out,
...
def handle_auto_state(detection):
    ...
    # 얼굴이 안정되었고, 눈이 열려 있으면 baseline EAR 수집
