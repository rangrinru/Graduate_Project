...
logger = logging.getLogger("rpicam_auto_v4")
...
def analyze_porphyrin_session(session_dir):
    ...
    clean = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR)
    clean[:] = 0
    ...
    for label_idx in range(1, num_labels):
        ...
        if area >= PORPHYRIN_MIN_AREA:
            clean[labels == label_idx] = (255, 255, 255)
            porphyrin_count += 1
            porphyrin_area += area

    clean_mask = cv2.cvtColor(clean, cv2.COLOR_BGR2GRAY)
...
def capture_sequence(cam, preview_config, still_config, exposure_ms, gain, save_as_png=True):
    ...
    try:
        ...
        for cam_key in ["cam2", "cam3", "cam4"]:
            ...
            show_single_camera(WINDOW_NAME, target_frame, cam_key, exposure_ms, hold_ms=250)
        ...
        if ANALYSIS_ENABLED:
            try:
                ...
                logger.exception("자동 분석 실패: %s", analysis_error)
...
def draw_face_guide(cam2_bgr, detection):
    ...
    cv2.putText(out, detection["guide_text"], (20, h - 130), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    cv2.putText(
        out,
        ...
        while True:
            ...
            if STATE["armed"]:
                ...
                if should_capture:
                    ...
                    if STATE["last_analysis_report"] is not None:
                        STATE["status"] = f"촬영+분석 완료 (P:{STATE['last_analysis_report']['porphyrin_count']})"
                    else:
                        STATE["status"] = "촬영 완료"
            else:
                white_led_off()
            ...
            if key == ord('a'):
                if STATE["last_session_dir"] is not None:
                    try:
                        analyze_porphyrin_session(STATE["last_session_dir"])
                        report = STATE["last_analysis_report"]
                        STATE["status"] = f"재분석 완료 (P:{report['porphyrin_count']})"
                    except Exception as e:
                        logger.exception("재분석 실패: %s", e)
                        STATE["status"] = "재분석 실패"
                else:
            ...
            if key == ord('p'):
                ...
                if STATE["last_analysis_report"] is not None:
                    STATE["status"] = f"즉시 촬영+분석 완료 (P:{STATE['last_analysis_report']['porphyrin_count']})"
                else:
                    STATE["status"] = "즉시 촬영 완료"
