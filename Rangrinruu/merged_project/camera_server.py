...
from porphyrin_analysis import analyze_session_dir, load_analysis_report
...
def get_capture_history(profile_id: str):
    ...
    for capture_dir in profile_root.iterdir():
        ...
        meta_path = capture_dir / "session_metadata.json"
        captured_at = None
        if meta_path.exists():
            try:
        ...
        history.append({
            ...
            "capturedAt": captured_at or capture_id,
...
def _build_image_item(profile_id: str, capture_id: str, cam_key: str, filter_type: str, display_name: str):
    profile_root = get_profile_root(profile_id)
    target_dir = profile_root / capture_id
    candidates = [
    ...
    meta_path = target_dir / f"{cam_key}_metadata.json"
    metadata = {}
    if meta_path.exists():
        try:
...
def resolve_image_path(profile_id: str, capture_id: str, filter_type: str) -> Path:
    ...
    candidates = [session_dir / f"{cam_key}.png", session_dir / f"{cam_key}.jpg", session_dir / f"{cam_key}.jpeg"]
...
def health():
    return jsonify({
        "ok": True,
        "camera_ready": capture_service.camera_ready,
        "analysis_in_progress": analysis_state["in_progress"],
    })
